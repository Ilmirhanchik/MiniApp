...
const App = () => {
  ...
  return (
    // Убран внешний контейнер min-h-screen для лучшего встраивания в мессенджер UI
    <div className="p-4 bg-white/70 backdrop-blur-sm"> 
      <div className="w-full max-w-lg bg-white shadow-lg border border-gray-100 rounded-xl p-6 md:p-8 mx-auto">
        <h1 className="text-2xl font-bold text-blue-700 mb-2 border-b pb-2">
          Подача Заявки (Переход на Госуслуги)
        </h1>
        <p className="text-gray-600 mb-6 text-sm">
          Введите информацию о вашей заявке. Для официального оформления вы будете перенаправлены на портал Госуслуги.
        </p>
        ...
        <button
          onClick={handleRedirectToGosuslugi}
          // Кнопка отключается только на время отображения сообщения
          disabled={!!message} 
          className={`w-full flex items-center justify-center px-6 py-3 border border-transparent text-base font-bold rounded-xl shadow-lg transition duration-300 ease-in-out transform hover:scale-[1.01] focus:outline-none focus:ring-4 focus:ring-blue-500/50
            ${
              message
